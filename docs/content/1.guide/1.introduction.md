...
- **Nested Table Gesture Arbitration** — Supports parent-child nested table scenarios (e.g., sub-tables inside expandable rows). Through passive tracking and mid-gesture edge passthrough, only one table instance responds to a gesture at a time, with zero-gap seamless handover from child to parent when the child reaches its scroll boundary.
