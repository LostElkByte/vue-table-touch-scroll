## [1.1.2](https://github.com/LostElkByte/vue3-mobile-table/compare/v1.1.1...v1.1.2) (2026-04-08)


### Features

* Nested Table Gesture Arbitration ([8a7fb07](https://github.com/LostElkByte/vue3-mobile-table/commit/8a7fb07d271ea58091eec1192756b7e22fc33fad))
## [1.1.1](https://github.com/LostElkByte/vue3-mobile-table/compare/v1.1.0...v1.1.1) (2026-04-03)
# [1.1.0](https://github.com/LostElkByte/vue3-mobile-table/compare/v1.0.6...v1.1.0) (2026-04-02)


### Features

*  Enhance documentation and examples for vue-table-touch-scroll ([85953b4](https://github.com/LostElkByte/vue3-mobile-table/commit/85953b4aafca133951cf6356fa3db0ea544267fb))
* add CSS rotation support to vTableTouchScroll directive ([c3b423c](https://github.com/LostElkByte/vue3-mobile-table/commit/c3b423c3bdf65a0babac039160fe02ef4cf2b2df))
* enhance directive with device detection and lazy hijacking ([b80ea64](https://github.com/LostElkByte/vue3-mobile-table/commit/b80ea64d60f6af31ec0d02fec5e675936835d3ce))
# [1.0.0](https://github.com/LostElkByte/vue3-mobile-table/compare/v1.0.6...v1.0.0) (2026-04-02)


### Features

*  Enhance documentation and examples for vue-table-touch-scroll ([85953b4](https://github.com/LostElkByte/vue3-mobile-table/commit/85953b4aafca133951cf6356fa3db0ea544267fb))
* add CSS rotation support to vTableTouchScroll directive ([c3b423c](https://github.com/LostElkByte/vue3-mobile-table/commit/c3b423c3bdf65a0babac039160fe02ef4cf2b2df))
* enhance directive with device detection and lazy hijacking ([b80ea64](https://github.com/LostElkByte/vue3-mobile-table/commit/b80ea64d60f6af31ec0d02fec5e675936835d3ce))
# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

### Added
- Initial release of vue3-mobile-table
- Touch scrolling directive for Vue 3 tables
- Direction locking (horizontal/vertical)
- Edge detection and bounce prevention
- Inertia scrolling with physics engine
- Smart click blocking during scroll
- TypeScript support
- Comprehensive test suite
- VitePress documentation
- CI/CD with GitHub Actions

### Features
- **Core Directive**: `v-mobile-table` for touch-based table scrolling
- **Customizable Options**: threshold, enabled state, custom selectors
- **Performance**: Normalized frame rate physics engine (60 FPS)
- **Accessibility**: Mouse and touch event support
- **Developer Experience**: Full TypeScript types, ESLint, Prettier

## [0.0.1] - TBD

### Initial Development
- Project setup with monorepo structure
- Core directive implementation
- Test infrastructure with Vitest
- Documentation with VitePress
- GitHub Actions workflows

---

## Release Notes

### How to Read This Changelog

- **Added**: New features
- **Changed**: Changes in existing functionality
- **Deprecated**: Soon-to-be removed features
- **Removed**: Removed features
- **Fixed**: Bug fixes
- **Security**: Security fixes

### Version Numbering

This project follows [Semantic Versioning](https://semver.org/):
- **MAJOR** version for incompatible API changes
- **MINOR** version for new functionality in a backwards compatible manner
- **PATCH** version for backwards compatible bug fixes

---

[Unreleased]: https://github.com/LostElkByte/vue3-mobile-table/compare/v0.0.3...HEAD
[0.0.1]: https://github.com/LostElkByte/vue3-mobile-table/releases/tag/v0.0.3
