 ...
 * @param options - 指令配置选项 / Directive configuration options
 *
 * 1. 备份 DOM 原生样式 / Backup native DOM styles
 * 2. 注入接管样式的 CSS 变量 / Inject style overrides
 * 3. 挂载事件总线与上下文管理 / Mount event listeners and context management
 */
function initDirective(el: HTMLElement, options: TableTouchScrollOptions) {
  ...
  // 1. 备份原始样式 / Backup original styles
  ...
  const ctx: ScrollContext = {
    ...
    activeTouchId: null, // 初始化追踪 ID / Initialize tracking ID
    touchTracker: [], // 初始化历史轨迹 / Initialize history trajectory
  }
...
function cleanup(el: HTMLElement) {
  ...
  ctx.abortController.abort() // 一键注销所有事件 / Cancel all event listeners
...
function onTouchStart(e: TouchEvent, ctx: ScrollContext) {
  if (e.touches.length > 1) {
    // 明确阻止原生滚动行为 / Explicitly prevent native multi-touch scroll
    if (e.cancelable) {
      e.preventDefault()
    }
    return // 忽略多指手势 / Ignore multi-finger gestures
  }
